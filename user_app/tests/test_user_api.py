...
class PublicUserApiTest(TestCase):
  ...
  def tets_user_exists(self):
    """ Probar que usuario que ya existe falla """
  ...
  def test_create_token_for_user(self):
    """ Probar que el token sea usado para el usuario """
  ...
  def test_create_token_invalid_credentials(self):
    ...
    payload = {"email":"test@gmail.com", "password":"fgjfj"}
    res = self.client.post(TOKEN_URL, payload)
  ...
  def test_create_token_missing_password(self):
    """ Probar que el correo y la contraseña sean requeridos """

    res = self.client.post(TOKEN_URL, {"email":"gtgret", "password": ""})
    self.assertNotIn("token", res.data)
    self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)
